...
Обратите внимание, что для доступа к Pinc000 API ваш аккаунт должен быть пополнен.

#### Форматы данных

Pinc000 API поддерживает только формат JSON.
...
Pinc000 API поддерживает HTTP-компрессию. Мы настоятельно рекомендуем использовать компрессию, так как это обеспечит наилучшую производительность.
...
# Статус API
Вы можете следить за [страницей статуса Pinc000](https://status.pinc000.com/) и подписаться на уведомления о статусе API. 


**[Спецификация Pinc000 API Open API ](https://github.com/pinc000/pinc000.github.io/tree/main/docs/OpenAPI)** размещена на GitHub.


# Предупреждение

 Pinc000 не несет ответственности за использование API для любых целей. API предоставляется «как есть» и «по мере доступности», без каких-либо гарантий, явных или подразумеваемых, включая, но не ограничиваясь, подразумеваемыми гарантиями товарной пригодности, пригодности для конкретной цели или отсутствия нарушений.
